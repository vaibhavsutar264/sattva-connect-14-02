...
class CompletedClass extends Component {
    constructor(props) {
        super(props);
        this.state = {
            userId: '',
    ...
    componentDidMount() {
        ...
        VideoServices.fetchMyFavoriteVideos(userId, this.state.limit, 0).then(
            (res) => {
                this.setState({
                    cursor: res.data.cursor,
                    hasMore: res.data.hasMore,
                    videos: res.data.markdone,
                    isloading: false,
                });
            }
        );
