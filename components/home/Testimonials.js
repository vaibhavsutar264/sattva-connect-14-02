      ...
      {testimonials && testimonials.length !== 0 ? (
        <section className='sec sec-testimonial text-center'>
          <div className='container'>
            ...
            <OwlCarousel
              id='sattvatestimonial'
              className='owl-carousel owl-theme testimonial-block'
              nav={true}
              items={1}
              dots={false}
            >
              {testimonials.map((item, index) => {
                return (
                  <div
                    className='row item align-items-center justify-content-center'
                    key={index}
                  >
                    <div className='col-lg-5 col-md-5 col-sm-12'>
                      <a
                        className='modal-trigger img-fluid'
                        data-toggle='modal'
                        data-target='#testimonial'
                        data-backdrop='static'
                        data-keyboard='false'
                        onClick={() => setTestimonialVideo(item.videoLink)}
                      >
