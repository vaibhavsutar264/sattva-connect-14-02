...
class BuyCourseFormWithotPayment extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    if (affiliate_id) {
      this.setState({ affiliateUrlId: affiliate_id });
    }
  }
  ...
  async onSubmit(e) {
    ...
    const userDetail = {
      ...
      affiliate_id: this.state.affiliateUrlId,
      courseId: this.props.courseId,
    ...
    CourseServices.buyCourseWithRegistrationWithoutPayment(userDetail)
      .then((res) => {
        window.scrollTo(0, 0);
  ...
  render() {
    const clintId = this.props.braintree;
    return (
      <>
        {this.state.redirectToThankyou && (
          <Redirect
            to={
              '/dharma-course-buy/' +
        ...
        <p>You get 3 Free Episodes, 3 Free Lessons, 3 Free Meditations by registering with your Name and Email.</p>
...
export default BuyCourseFormWithotPayment;
